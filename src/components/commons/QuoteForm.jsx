    ...
    return (
        <div className="grid grid-cols-4 gap-4 p-4 rounded-sm border border-stroke bg-white px-5 pt-7.5 pb-5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:col-span-8">
            <div className="col-span-2">
                <Select options={[{ label: "Selecciona el cliente", value: "" }, ...clientOptions]} title="Nombres y Apellidos" onChange={e => { onChangeInput(e, 'customer') }} value={detail?.customer} />
            </div>
            <div className="col-span-2">
            ...
            <Input title="Nº días"
                onBlur={e => { onFocusOut(e, 'days') }}
                onFocus={e => { onFocus(e, 'days') }}
                type="number"
                min={1}
                value={detail.days}
                onChange={e => { onChangeInputNumber(e, 'days') }}
            />
            <Input
                title="Nº Adultos"
                onBlur={e => { onFocusOut(e, 'adults') }}
                onFocus={e => { onFocus(e, 'adults') }}
                min={1}
                value={detail.adults}
                onChange={e => { onChangeInputNumber(e, 'adults') }}
                type={'number'}
            />
            <Input title="Nº Niños "
                onBlur={e => { onFocusOut(e, 'kids') }}
                onFocus={e => { onFocus(e, 'kids') }}
                min={0}
                value={detail.kids}
                onChange={e => { onChangeInputNumber(e, 'kids') }}
                type={'number'} />


            <Input title="fee Adulto"
                onBlur={e => { onFocusOut(e, 'adultFee') }} o
                nFocus={e => { onFocus(e, 'adultFee') }}
                type="number" min={100}
                value={detail.adultFee}
                onChange={e => { onChangeInputNumber(e, 'adultFee') }} />
