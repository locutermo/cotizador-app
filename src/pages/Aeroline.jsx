...
export default function Aerolines() {
    ...
    const aerolines = useSelector(aerolinesFormattedSelector)
    ...
    const headers = [
        ...
        },
        {
            ...
            callbacks: [
                {
                    Component: (props) => (<button {...props}> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-5">
    ...
    return (
        <>
            <Breadcrumb current={{ name: 'Aerolineas' }} previous={{ name: "Inicio", url: '' }} />
            <div className="flex gap-4">
                <div className="w-3/12 flex flex-col gap-4 h-auto">
                    <Form title="Creación" inputs={inputs} callback={(data) => { dispatch(addAeroline(data)) }} />
                    {selected && (
                        <Form
                            title="Edición"
                ...
                <div className="w-9/12">
                    <Table data={aerolines} headers={headers} />
                </div>
            </div>
